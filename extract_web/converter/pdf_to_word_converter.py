...
def convert_pdf_to_word(input_pdf_path, output_docx_path, mode="pdf2docx"):
    ...
    try:
        logger.info(
            f"Starting PDF to Word conversion using {mode} mode: {input_pdf_path} -> {output_docx_path}"
        )
        ...
        logger.error(
            f"Error during PDF to Word conversion for '{input_pdf_path}' using {mode}: {e}",
            exc_info=True,
        )
...
def _convert_pdf_to_word_pdf2docx(input_pdf_path, output_docx_path):
    ...
    try:
        ...
        if os.path.exists(output_docx_path):
            logger.info(
                f"Successfully converted PDF to Word using pdf2docx: {output_docx_path}"
            )
...
def _convert_pdf_to_word_libreoffice(input_pdf_path, output_docx_path):
    ...
    try:
        ...
        logger.info(
            f"Using LibreOffice to convert {input_pdf_path} to {output_docx_path}"
        )
...
def convert_and_merge_pdfs_to_docx(
    pdf_paths, merged_docx_path, request_id="", mode="pdf2docx"
):
    """Converts multiple PDFs to DOCX and merges them into a single DOCX file."""
    logger.info(
        f"Starting convert_and_merge_pdfs_to_docx for {len(pdf_paths)} files. Output: {merged_docx_path}. RequestID: {request_id}"
    )
    ...
    for i, pdf_path in enumerate(pdf_paths):
        ...
        success, actual_output_path, error_msg = convert_pdf_to_word(
            pdf_path, temp_docx_path, mode=mode
        )

        if success and actual_output_path and os.path.exists(actual_output_path):
            temp_docx_files.append(actual_output_path)
            logger.info(
                f"Successfully converted {pdf_path} to {actual_output_path} for merging. RequestID: {request_id}"
            )
        else:
            any_conversion_failed = True
            conversion_errors.append(
                error_msg or f"Failed to convert {os.path.basename(pdf_path)}"
            )
            logger.error(
                f"Failed to convert {pdf_path} to DOCX for merging. Error: {error_msg}. RequestID: {request_id}"
            )

    if not temp_docx_files:
        logger.error(
            f"No PDF could be converted to DOCX for merging. Errors: {"; ".join(conversion_errors)}. RequestID: {request_id}"
        )
        return False, (
            "; ".join(conversion_errors)
            if conversion_errors
            else "No PDF files could be converted to DOCX."
        )
    ...
    try:
        ...
        for i in range(1, len(temp_docx_files)):
            ...
            logger.info(
                f"Appended {temp_docx_files[i]} to merged document. RequestID: {request_id}"
            )

        merged_doc.save(merged_docx_path)
        logger.info(
            f"Successfully merged {len(temp_docx_files)} DOCX files into {merged_docx_path}. RequestID: {request_id}"
        )

        final_message = (
            f"Successfully merged {len(temp_docx_files)} PDF(s) into a DOCX file."
        )
        ...
        logger.error(
            f"Error during DOCX merge process for {merged_docx_path}: {e_merge}. RequestID: {request_id}",
            exc_info=True,
        )
        ...
        for temp_file in temp_docx_files:
            if os.path.exists(temp_file):
                try:
                    os.remove(temp_file)
                    logger.info(
                        f"Cleaned up temporary file: {temp_file}. RequestID: {request_id}"
                    )
                except Exception as e_clean:
                    logger.warning(
                        f"Failed to clean up temporary file {temp_file}: {e_clean}. RequestID: {request_id}"
                    )
