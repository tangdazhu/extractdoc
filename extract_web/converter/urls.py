...
urlpatterns = [
    ...
    path(
        "admin-console/file-management/",
        views.admin_file_management,
        name="admin_file_management",
    ),
    path(
        "admin-console/user/delete/<int:user_id>/",
    ...
    path(
        "download_file/<str:request_id>/<str:filename>/",
        views.download_converted_file_view,
        name="download_file",
    ),
    # path('api/analyze_document', views.analyze_document_view, name='analyze_document'),  # Example for a potential new API endpoint
    path(
        "api/video-to-extraction/",
        views.process_video_extraction_view,
        name="api_video_to_extraction",
    ),  # ADDED for new video processing
    path("history/", views.conversion_history_view, name="conversion_history"),
    path(
        "delete-converted-file/<str:date_str>/<str:filename>/",
        views.delete_converted_file_view,
        name="delete_converted_file",
    ),
    path(
        "history/delete_all_for_date/<str:date_str>/",
        views.delete_all_for_date_view,
        name="delete_all_for_date",
    ),
    path(
        "download/<str:username>/<str:date_str>/<str:filename>/",
        views.download_converted_file_view,
