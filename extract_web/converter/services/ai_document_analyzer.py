...
4. **元数据表判断** - 哪些表格是元数据表(版本信息、作者、团队等)?
   - **关键规则**: 元数据表通常在**标题页**(第1页),包含Team/Version/Date等信息
   - **元数据表特征**: 2-3列,3-5行,包含键值对(如"Team: xxx", "Version: 0.5")
   - **内容表特征**: 多列(5+列),多行(5+行),包含详细数据(如更新记录、版本历史)
   - **重要**: 如果第1页有小表格(2-3列),第2页有大表格(5+列),则第1页的表格是元数据表,第2页的表格是内容表
  ...
  "title_page": {{
    ...
    "elements": {{
      ...
      "metadata_table": {{
        ...
        "purpose": "团队和版本信息",
        "should_include": true,
        "note": "元数据表必须在第1页,包含Team/Version/Date等信息"
      }}
    }}
  }},
  "content_pages": [
