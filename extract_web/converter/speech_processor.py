...
def transcribe_audio_dashscope(
    audio_file_url: str, hotwords_config: list[dict] = None
) -> dict:
    """
    ...
    try:
        logger.debug(
            f"Current DashScope API Key (at point of call): {dashscope.api_key[:5] if dashscope.api_key else 'Not Set'}"
        )

        current_model = "paraformer-v2"
        call_params = {"model": current_model, "file_urls": [audio_file_url]}
        ...
        if hotwords_config and isinstance(hotwords_config, list) and hotwords_config:
            ...
            for item in hotwords_config:
                if not (
                    ...
                    valid_config = False
                    logger.warning(
                        f"Invalid item in hotwords_config: {item}. Expected dict with text, weight, lang."
                    )
                    break

            if valid_config:
                logger.info(
                    f"Hotwords config provided: {hotwords_config}. Attempting to create and use a dynamic vocabulary."
                )
                try:
                    ...
                    logger.info(
                        f"Creating vocabulary with target_model='{current_model}', vocabulary={hotword_definitions}, prefix='utvocab'"
                    )
                    ...
                    if created_vocab_id:
                        ...
                        logger.info(
                            f"Successfully created and will use vocabulary_id: {vocabulary_id_to_use}"
                        )
                        if (
                            "language_hints" in call_params
                        ):  # Should not be there anymore, but defensive
                            del call_params["language_hints"]
                    else:
                        logger.error(
                            "Failed to create dynamic vocabulary, create_vocabulary returned a falsy ID."
                        )
                except Exception as e_vocab:
                    logger.error(
                        f"Error creating dynamic vocabulary for hotwords_config {hotwords_config}: {e_vocab}",
                        exc_info=True,
                    )
            else:
                logger.warning(
                    "Hotwords config was provided but was invalid. Proceeding without dynamic vocabulary."
                )
        else:
            logger.info(
                "No valid hotwords_config provided. Not creating or using dynamic vocabulary."
            )
        ...
        if not task_response or task_response.status_code != HTTPStatus.OK:
            logger.error(
                f"Failed to submit transcription task. Response: {task_response}"
            )
            error_message = (
                task_response.message
                if task_response and hasattr(task_response, "message")
                else "Unknown error submitting task."
            )
            return {
                "status": "error",
                "message": f"Failed to submit transcription task: {error_message}",
            }
        ...
        if transcription_response.status_code == HTTPStatus.OK:
                ...
                if transcription_url:
                    try:
                        with requests.get(transcription_url, stream=True) as r:
                            ...
                            logger.info(
                                f"Fetched transcription JSON from {transcription_url}: {result_content[:500]}"
                            )
                        ...
                        logger.error(
                            f"Error fetching or parsing transcription result from URL {transcription_url}: {e}"
                        )
                else:
                    if "message" in result_item:
                        logger.warning(
                            f"No transcription_url found in result item. DashScope subtask message: '{result_item.get('message')}' (Code: {result_item.get('code')}, Status: {result_item.get('subtask_status')}) - Item: {result_item}"
                        )
                        if (
                            result_item.get("message")
                            == "Failed to convert json string to java object list!"
                        ):  # Still check for this specific past error
                            return {
                                "status": "error",
                                "message": f"DashScope ASR subtask failed (JSON conversion): {result_item.get('message')}",
                                "raw_response": transcription_response.output,
                            }
                    else:
                        logger.warning(
                            f"No transcription_url found in result item and no specific message: {result_item}"
                        )
            ...
            logger.error(
                f"Transcription failed. Status Code: {transcription_response.status_code}, Response: {transcription_response}"
            )
            error_message = "Unknown transcription error."
            raw_output_message = None
            if (
                hasattr(transcription_response, "output")
                and transcription_response.output
                and hasattr(transcription_response.output, "message")
            ):
                raw_output_message = transcription_response.output.message
            if raw_output_message:
                error_message = raw_output_message
            elif (
                hasattr(transcription_response, "message")
                and transcription_response.message
            ):
            ...
            return {
                ...
                "raw_response": (
                    transcription_response.output
                    if hasattr(transcription_response, "output")
...
if __name__ == "__main__":
    ...
    if not dashscope.api_key and not os.environ.get("DASHSCOPE_API_KEY"):
        print(
            "Please set the DASHSCOPE_API_KEY environment variable or dashscope.api_key directly to test."
        )
    else:
        if (
            not dashscope.api_key
        ):  # If not set directly, try to use env var for the test print
            print(
                f"DASHSCOPE_API_KEY found from env: {os.environ.get('DASHSCOPE_API_KEY')[:5]}..."
            )
        ...
        result_with_hotwords = transcribe_audio_dashscope(
            test_audio_url_m4a, hotwords_config=test_hotwords_config_example
        )
