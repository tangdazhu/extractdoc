...
变更日志（2025-06-21）
-----------------------
- 新增：实时语音识别流式同步，支持 DashScope 临时/最终结果实时推送，前端可区分 is_final 渲染。
- 优化：会话管理与权限校验，所有识别结果通过 get_all_results() 统一推送，避免丢失和重复。
- 增强：详细日志，便于排查同步与计数问题。
- 兼容：支持中英文，兼容 DashScope WebSocket/HTTP 接口。
- 文档：补充详细设计说明与踩坑记录，便于团队维护。

【模块功能概述】
本模块为“智能文档转换工作室”提供实时语音识别的 Django 视图与 WebSocket 消费者，负责会话管理、音频数据转发、识别结果推送、权限校验等。支持 DashScope 流式接口，兼容中英文，前后端流式同步。

【设计说明与踩坑记录】
--------------------------------------
1. 采用 session_id 进行多会话隔离，所有识别器、结果、用户信息均按 session_id 存储，便于多用户并发。
2. 识别结果通过 recognizer.get_all_results() 统一返回所有新内容，前端每次轮询都能拿到所有临时+最终结果，避免丢失和重复。
3. 早期方案直接用 Python list 存储结果，存在多线程/异步下丢失、竞争等问题，现已全部由 recognizer 内部统一管理。
4. WebSocket 和 HTTP API 均支持实时推送，WebSocket 端支持自动翻译，HTTP 端支持轮询。
5. 权限校验：所有操作均校验用户身份，防止越权访问。
6. 日志设计：所有关键操作、异常、识别结果、同步点均有详细日志，便于排查。
7. 最佳实践总结：
