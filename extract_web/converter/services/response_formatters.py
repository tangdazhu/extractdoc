...
def format_json_response(
    ...
    if error_message and not results: # If it's a general error before processing any file
        ...
        logger.warning(f"JsonResponse being formatted with an error_message but also existing results. Error: {error_message}. RequestID: {request_id}")
...
def format_error_response(
    ...
    """
    error_result = [{
        'original_name': original_item_name,
        'status': 'error',
        'message': message
    }]
